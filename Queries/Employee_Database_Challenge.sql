...
ORDER BY e.emp_no;
-- 10. Export mentorship_eligibilty table - done
-- 11. Confirm with sample image - done **Error in module sample image for emp_no 10291. Title should be Senior Staff.


-- Additional queries
-- 1. Sum of salaries for employees retiring by title
-- Sum of salaries for employees within unique_titles table
SELECT SUM (s.salary)
FROM salaries AS s
INNER JOIN unique_titles AS ut
ON (s.emp_no = ut.emp_no);

-- Sum of salaries for employees within unique_titles table by title
SELECT ut.title,
	COUNT(ut.emp_no), 
	SUM (s.salary) 
INTO salaries_by_title
FROM unique_titles AS ut
...
-- 2. Potential expansion of mentorship program to employees born 1964-1966
SELECT DISTINCT ON(e.emp_no) e.emp_no,
	e.first_name,
	e.last_name,
	e.birth_date,
